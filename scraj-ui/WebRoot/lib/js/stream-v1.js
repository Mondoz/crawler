...
(function(){
				...
				case "drop" :
					...
					if (bFolder) {
						...
						if (items.length && items[items.length - 1]) {
							var dropEnableFolder = this.get("dropEnableFolder");
							var entry = items[items.length - 1].webkitGetAsEntry() || items[items.length - 1].getAsEntry();
							(dropEnableFolder || (!dropEnableFolder && entry.isFile)) && entry && this.traverseFileTree(entry.filesystem.root, callback);
						}
					} else {
	...
	function Main(cfg){
		...
		this.config = {
			...
			dropEnableFolder : cfg.dropEnableFolder || true
		};
		Parent.apply(this, arguments);
	}
